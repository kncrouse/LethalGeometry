...
to data
  if year = hour  [
    ...
    ask groups [
      let me self
      let m death-count / ( count animals with [ parent-group = me ] + 0.000001 )
      let t count patches with [ pcolor = ( [color] of me - 4 ) ]
      print ( word m " " t )
      update-territory-mortality-graph t m
      update-territory-mortality-graph random 1000 random-float 1.0
      ;print death-count
      ;print territory
      set death-count 0
    ]
...
to check-dying
  if dying? [
    ...
    die ]
  ask groups [ if ( count animals with [ parent-group = myself ] = 0 ) [ die ]]
end
...
to update-territory-mortality-graph [territory mortality]
  let radius ( sqrt (territory / pi) );

  if mortality > 0 [
    set-current-plot "Mort-Terr";
    plotxy (1 / ( radius + 0.0000001 )) mortality;
    plot-pen-down;
    plot-pen-up;
  ]
end
